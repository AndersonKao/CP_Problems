...
int main(){
	...
	while( cin >> n ){
		...
		for(int i = 0; i < py.size(); i++){
			int j = (i + 1 == py.size() ? 0 : i+1)	;
			Line<double>L(Point<double>(py[i]), Point<double>(py[j]));
			double large = 0.0;
			for(int k = 0; k < py.size(); k++){
				large = max(L.disP2Line(Point<double>(py[k])), large);
			}	
			ans = min(ans, large);
		}
		ans = sqrt(ans);
		cout << "Case " << caseN++ << ": " << fixed << setprecision(2) << (ans * 100.0) / 100.0	 << endl;
